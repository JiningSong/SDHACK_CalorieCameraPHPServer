...
class ClarifaiIOSController extends Controller
{
    public function index(Request $request) {
        ...
        $img = base64_decode($request->get('picture'));

        Storage::disk('local')->put('avatars/1.jpg', $img);

        $client = new ClarifaiClient('5eaae2c6f2554ec3ade9885d5e077936');

        $response = $client->workflowPredict('20001023',
            new ClarifaiFileImage(file_get_contents('storage/avatars/1.jpg')))
            ->executeSync();
...
//                print_r($J);
                foreach ($J->branded as $name) {
                    if (strcasecmp($name->food_name, $food_name) == 0) {
                        if($name->nf_calories != null) {
                            return [$name->food_name, $name->nf_calories / $name->serving_qty, $name->serving_unit];
                        }
                        else {
                            continue;
                        }
                        ...
                        if($name->nf_calories != null) {
                            return [$name->food_name, $name->nf_calories, $name->serving_unit];
                        }
                        else {
                            continue;
                        }
                ...
                echo "Warning: No such food found";
            }
        }
...
//            $str = file_get_contents($request->Photo);
